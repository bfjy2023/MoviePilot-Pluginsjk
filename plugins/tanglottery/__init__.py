...
class TangLottery(_PluginBase):
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warn("不可躺自动抽奖助手 Cron 配置无效，定时服务未注册")
            return []
        return [
            {
                "id": "TangLottery",
    ...
    def run_lottery_task(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not cookie or "c_secure_pass=" not in cookie:
                logger.warn("抽奖任务终止：缺少包含 c_secure_pass 的 不可躺 Cookie")
                result = self.__new_result(status="auth_failed", message="缺少包含 c_secure_pass 的 不可躺 Cookie")
                self.__finish_task(result)
                return result

            target_count = self.__safe_int(self._target_count, 100, min_value=1)
    ...
    def __post_spin(self, count: int, cookie: str) -> Tuple[Optional[dict], Optional[str], str]:
        ...
        if response.status_code in {401, 403}:
            logger.warn(f"不可躺抽奖接口权限错误：count={count}，HTTP {response.status_code}，响应={self.__to_log_text(text)}")
        ...
        if data.get("ok") is False:
            ...
            logger.warn(f"不可躺抽奖接口返回失败：count={count}，message={message}")
        ...
        if data.get("ok") is not True:
            ...
            logger.warn(f"不可躺抽奖接口返回未知状态：count={count}，message={message}，data={self.__to_log_text(data)}")
            return data, "request_failed", message
    ...
    def __fetch_lottery_info(self) -> Dict[str, Any]:
        ...
        if not cookie or "c_secure_pass=" not in cookie:
            ...
            logger.warn("读取 不可躺抽奖页面失败：缺少包含 c_secure_pass 的 Cookie")
            return info
        ...
        if response.status_code in {401, 403}:
            ...
            logger.warn(f"读取 不可躺抽奖页面权限错误：HTTP {response.status_code}")
            return info
        if response.status_code != 200:
            info["message"] = f"读取抽奖页面失败：HTTP {response.status_code}"
            logger.warn(f"读取 不可躺抽奖页面失败：HTTP {response.status_code}，响应预览={self.__to_log_text(response.text or '')}")
            return info
        ...
        if any(word in plain_text for word in ["Cookie失效", "非法访问", "请先登录", "未登录"]):
            ...
            logger.warn(f"读取 不可躺抽奖页面返回登录/权限提示，页面文本预览={self.__to_log_text(plain_text)}")
            return info

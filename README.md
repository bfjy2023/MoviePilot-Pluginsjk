...
| `ForumRssMonitor` | 论坛动态监控 | `1.0.0` | 监控论坛 RSS/Atom 动态，并按关键词推送新帖。 |
...
MoviePilot V2 插件，用于监控论坛 RSS/Atom 动态，并按关键词推送新帖。

主要能力：

- RSS 地址列表使用多行文本框配置，一行一个链接
- 支持 Atom 和 RSS 2.0 常见字段解析
- 标题、作者、摘要任一字段命中关键词即推送
- 关键词留空时推送全部新条目
- 首次运行会推送命中关键词的现有条目，后续通过已见 ID 去重
